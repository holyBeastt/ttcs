...
const importWorkloadToDB = async (req, res) => {
  try {
    ...
    const query = `
      ...
      (giang_vien, khoa, hoc_ky, nam_hoc, hinh_thuc, ten_hoc_phan, lop_hoc_phan, doi_tuong, bai_cham_1, bai_cham_2, tong_so, quy_chuan, khoa_duyet, khao_thi_duyet)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
...
const deleteWorkloadData = async (req, res) => {
  try {
    ...
    const deleteQuery = `
      DELETE FROM ${COI_CHAM_RA_DE_TABLE}
...
const getList = async (req, res) => {
  ...
  try {
    ...
    conn = await createPoolConnection();
    console.log("MaPhongBan:", MaPhongBan, "Ki:", Ki, "Nam:", Nam);

    const query = `
      SELECT ${LEGACY_SELECT} FROM ${COI_CHAM_RA_DE_TABLE}
...
const insertMyData = async (req, res) => {
  try {
    ...
    const query = `
      INSERT INTO ${COI_CHAM_RA_DE_TABLE}
      (giang_vien, khoa, hoc_ky, nam_hoc, hinh_thuc, ten_hoc_phan, lop_hoc_phan, doi_tuong, bai_cham_1, bai_cham_2, tong_so, quy_chuan)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
...
const updateMyData = async (req, res) => {
  try {
    ...
    for (const item of dataList) {
      ...
      const getOldDataQuery = `SELECT ${LEGACY_SELECT} FROM ${COI_CHAM_RA_DE_TABLE} WHERE id = ?`;
      ...
      const query = `
        UPDATE ${COI_CHAM_RA_DE_TABLE}
        SET ten_hoc_phan = ?, lop_hoc_phan = ?, hinh_thuc = ?, bai_cham_1 = ?, bai_cham_2 = ?, tong_so = ?, quy_chuan = ?
        WHERE id = ?
      `;
...
const deleteMyData = async (req, res) => {
  try {
    ...
    const getDataQuery = `SELECT ${LEGACY_SELECT} FROM ${COI_CHAM_RA_DE_TABLE} WHERE id = ?`;
...
const updateData = async (req, res) => {
  try {
    ...
    for (const item of dataList) {
      const { loai, id, tenHocPhan, lopHocPhan } = item; // Lấy loai và id từ từng phần tử trong mảng
      let section, soBaiCham1, soBaiCham2, tongSoBai, soTietQC, khoaduyet, khaothiduyet;
      ...
      const query = `
        UPDATE ${COI_CHAM_RA_DE_TABLE}
        SET ten_hoc_phan = ?, lop_hoc_phan = ?, hinh_thuc = ?, bai_cham_1 = ?, bai_cham_2 = ?, tong_so = ?, quy_chuan = ?, khoa_duyet = ?, khao_thi_duyet = ?
        WHERE id = ?
      `;
