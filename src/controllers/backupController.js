...
const resetDatabase = async (req, res) => {
    try {
        ...
        // Nếu không có file nào, báo lỗi
            ...
            return res.status(400).json({ success: false, message: "⚠️ Vui lòng chọn file SQL!" });
        }
    ...
    try {
        ...
        for (let triggerQuery of triggerQueries) {
            ...
            try {
                ...
                console.error(`❌ Lỗi khi chạy TRIGGER:\n${triggerQuery}\n⛔ Lỗi:`, err.message);
                return res.status(500).json({ 
                    success: false, 
        ...
        res.json({ success: true, message: "✅ Database đã được reset và nhập dữ liệu thành công!" });
    } catch (error) {
...
const listBackupFiles = async (req, res) => {
    try {
        if (!fs.existsSync(BACKUP_DIR)) {
            return res.status(404).json({ success: false, message: "Thư mục backup không tồn tại!" });
        }
...
const executeSQLQuery = async (req, res) => {
    try {
        ...
        if (!sqlQuery || sqlQuery.trim() === "") {
            return res.status(400).json({ success: false, message: "⚠️ Vui lòng nhập câu lệnh SQL!" });
        }
        ...
        if (forbiddenKeywords.some(keyword => sqlQuery.toUpperCase().includes(keyword))) {
            return res.status(403).json({ success: false, message: "⛔ Lệnh SQL không được phép!" });
        }
