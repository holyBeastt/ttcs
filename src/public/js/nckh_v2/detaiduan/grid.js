...
function renderTable() {
    ...
    // Quyền sửa: có quyền duyệt và item chưa được duyệt
    const canEdit = canApprove;
    
    // Quyền xóa: có quyền duyệt HOẶC là gv_cnbm_khoa hoặc lanhDao_khoa
    const canDelete = canApprove || role === gvCnbmKhoaRole || role === lanhDaoKhoaRole;
    
    // User chỉ xem (gv, thuong)
    const isViewOnly = role === gvRole || role === thuongRole;
...
async function onCellValueChanged(event) {
    ...
    try {
        const response = await fetch(`/v2/nckh/edit/${data.ID}/detaiduan`, {
            method: "POST",
...
async function deleteRow(id, api, node) {
    ...
    try {
        const response = await fetch(`/v2/nckh/delete/${id}/${currentYear}/detaiduan`, {
            method: "POST"
        });
...
async function toggleApproval(id, newStatus, api, node) {
    try {
        const response = await fetch(`/v2/nckh/update/${id}/${currentYear}/detaiduan`, {
            method: "POST",
            headers: {
